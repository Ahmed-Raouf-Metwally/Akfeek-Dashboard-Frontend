...
function WorkshopCard({ item, t }) {
    ...
    return (
        <div>
            <Link
                to={detailPath}
                className="group block overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition-all duration-200 hover:border-blue-200 hover:shadow-lg hover:shadow-blue-500/10"
            >
                <div className="relative aspect-[4/3] overflow-hidden bg-slate-100">
                    ...
                    <div className="absolute left-3 top-3">
                        <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider backdrop-blur-md ${isWorkshop ? 'bg-indigo-500/90 text-white' : 'bg-blue-500/90 text-white'}`}>
                            {isWorkshop ? (t('workshops.certified', 'Certified')) : (t('vendors.carWash', 'Car Wash'))}
                        </span>
                    </div>
                ...
                <div className="p-5">
                    <div className="mb-2">
                        <h3 className="line-clamp-1 font-bold text-slate-900 group-hover:text-blue-600">
                            {name}
                        </h3>
                        {nameAr && (
                            <p className="line-clamp-1 text-sm text-slate-500">{nameAr}</p>
                        )}
                    </div>
                    ...
                    <div className="mb-4 flex flex-wrap gap-y-2 text-sm text-slate-600">
                        ...
                        <div className="flex w-full items-center gap-1.5">
                            ...
                            <span className="text-xs text-slate-400">
                                ({item.totalReviews || 0} {t('workshops.reviews')})
                            </span>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center justify-between border-t border-slate-100 pt-3">
                        ...
                        <span className="inline-flex items-center gap-1 text-sm font-medium text-blue-600 group-hover:gap-2">
...
export default function WorkshopsCarWashPage() {
    ...
    const filteredEntities = useMemo(() => {
        const filteredWorkshops = workshops.filter(w => {
            ...
            return services.includes('wash') || services.includes('غسيل') || services.includes('تنظيف');
        });

        const merged = [...filteredWorkshops, ...washVendors];
    ...
    const { paginatedItems, totalPages, total } = useMemo(() => {
        ...
        const paginatedItems = filteredEntities.slice(start, start + PAGE_SIZE);
        return { paginatedItems, totalPages: pages, total: totalCount };
    ...
    if (!isAdmin) {
        return (
            <div className="flex flex-col items-center justify-center rounded-2xl border border-slate-200 bg-white py-20 text-center">
                ...
                <p className="mt-2 text-slate-500">{isAr ? 'صفحة للمسؤولين فقط.' : 'Admins only.'}</p>
            </div>
        );
    }
    ...
    return (
        <div className="space-y-8">
            <div
                className="overflow-hidden rounded-2xl bg-gradient-to-br from-blue-600 via-blue-700 to-indigo-800 p-8 shadow-xl shadow-blue-500/20"
            >
                    ...
                    <div className="flex items-start gap-5">
                        ...
                        <div>
                            <h1 className="text-2xl font-bold tracking-tight text-white sm:text-3xl">
                                {isAr ? 'ورش غسيل السيارات' : 'Car Wash Workshops'}
                            </h1>
                            <p className="mt-2 max-w-xl text-blue-100/90">
                                {isAr ? 'عرض كافة الفيندوز والورش التي توفر خدمات الغسيل.' : 'View all vendors and workshops providing car wash services.'}
                            </p>
                        </div>
                    </div>
                </div>
            ...
            <div className="space-y-6">
                <div className="flex flex-col gap-4 rounded-xl bg-white p-4 shadow-sm sm:flex-row sm:items-center">
                    <div className="relative flex-1">
                        ...
                        <input
                            ...
                            className="w-full rounded-lg border border-slate-200 py-2 pl-10 pr-3 text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"
                        />
                    </div>
                    <div className="text-sm text-slate-500">
                        {isAr ? `تم العثور على ${total} كيان` : `Found ${total} entities`}
                    </div>
                </div>
                ...
                {isLoading ? (
                    ...
                    <div className="flex flex-col items-center justify-center rounded-2xl border-2 border-dashed border-slate-200 bg-slate-50 py-16 text-center">
                        <Droplets className="mb-4 size-14 text-slate-300" />
                        <p className="text-slate-600">
                            {isAr ? 'لا توجد كيانات غسيل حالياً' : 'No washing entities found'}
                        </p>
                    </div>
                ) : (
                    <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                        {paginatedItems.map((item) => (
                            <WorkshopCard key={item.id} item={item} t={t} />
                        ))}
                    </div>
                )}
