...
export default function MarketplaceOrderDetailPage() {
  ...
  const updateStatusMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success(t('marketplaceOrders.detail.orderStatusUpdated'));
    },
    onError: () => toast.error(t('marketplaceOrders.detail.updateStatusFailed'))
  });
  ...
  const updateItemStatusMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries(['marketplace-order', id]);
      toast.success(t('marketplaceOrders.detail.itemStatusUpdated'));
    },
    onError: () => toast.error(t('marketplaceOrders.detail.updateStatusFailed'))
  });

  if (isLoading) return <div className="p-8 text-center">{t('marketplaceOrders.detail.loading')}</div>;
  if (!order) return <div className="p-8 text-center">{t('marketplaceOrders.detail.notFound')}</div>;
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      ...
      >
        <ArrowLeft className="w-4 h-4" /> {t('marketplaceOrders.detail.backToOrders')}
      </button>

      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
        <div>
          <h1 className="text-2xl font-bold text-slate-800 flex items-center gap-3">
            {t('marketplaceOrders.orderNumber')} {orderData.orderNumber}
            <span className={`text-sm px-3 py-1 rounded-full bg-slate-100 text-slate-600 font-medium`}>
              {t(`finance.status.${orderData.status}`, orderData.status)}
            </span>
          </h1>
          <p className="text-slate-500 text-sm mt-1">
            {t('marketplaceOrders.detail.placedOn')} {new Date(orderData.createdAt).toLocaleString()}
          </p>
        </div>
        
        <div className="flex flex-wrap gap-3">
          <Link
            to={`/marketplace-orders/${id}/invoice`}
            className="flex items-center gap-2 px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors"
          >
            <FileText className="w-4 h-4" /> {t('marketplaceOrders.detail.viewInvoice')}
          </Link>
          {isAdmin && (
            <select 
            ...
            >
              {STATUS_OPTIONS.map(s => (
                <option key={s} value={s} className="text-black">{t(`finance.status.${s}`, s)}</option>
              ))}
            </select>
          )}
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
            <h2 className="text-lg font-semibold mb-4 flex items-center gap-2">
              <Package className="w-5 h-5 text-indigo-500" /> {t('marketplaceOrders.detail.orderItems')} ({items.length})
            </h2>
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex justify-between items-end mt-2">
                      <div className="text-sm text-slate-600">
                        {t('marketplaceOrders.detail.qty')}: {item.quantity} × {item.unitPrice} SAR
                      </div>
                      ...
                      <div className="flex items-center gap-2">
                        {isVendor ? (
                          ...
                          >
                             {STATUS_OPTIONS.map(s => (
                                <option key={s} value={s}>{t(`finance.status.${s}`, s)}</option>
                              ))}
                          </select>
                        ) : (
                          <span className="text-xs px-2 py-1 bg-white border rounded text-slate-600">
                             {t('marketplaceOrders.detail.itemStatus')}: {t(`finance.status.${item.status}`, item.status)}
                          </span>
                        )}
            ...
            <div className="mt-6 border-t pt-4 space-y-2">
              <div className="flex justify-between text-slate-600">
                <span>{t('marketplaceOrders.detail.subtotalInclusive')}</span>
                <span>{Number(orderData.subtotal ?? 0).toLocaleString('en-US')} SAR</span>
              </div>
              <div className="flex justify-between text-slate-600">
                <span>{t('marketplaceOrders.detail.shipping')}</span>
                <span>{Number(orderData.shippingCost ?? 0).toLocaleString('en-US')} SAR</span>
              </div>
              <div className="flex justify-between text-lg font-bold text-slate-900 pt-2 border-t">
                <span>{t('marketplaceOrders.detail.total')}</span>
                <span>{(Number(orderData.subtotal ?? 0) + Number(orderData.shippingCost ?? 0)).toLocaleString('en-US')} SAR</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-6">
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
            <h2 className="text-lg font-semibold mb-4 flex items-center gap-2">
              <User className="w-5 h-5 text-indigo-500" /> {t('marketplaceOrders.detail.customerDetails')}
            </h2>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
            <h2 className="text-lg font-semibold mb-4 flex items-center gap-2">
              <MapPin className="w-5 h-5 text-indigo-500" /> {t('marketplaceOrders.detail.shippingAddress')}
            </h2>
            <div className="text-sm text-slate-600 space-y-1">
              <p className="font-medium text-slate-800">{orderData.recipientName || 'N/A'}</p>
              <p>{orderData.shippingAddress || t('marketplaceOrders.detail.noAddress')}</p>
              <p>{orderData.shippingCity ?? ''}, {orderData.shippingCountry ?? ''}</p>
              <p className="mt-2 text-slate-500">{t('marketplaceOrders.detail.phone')}: {orderData.recipientPhone || 'N/A'}</p>
            </div>
          </div>

          <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
            <h2 className="text-lg font-semibold mb-4 flex items-center gap-2">
              <CreditCard className="w-5 h-5 text-indigo-500" /> {t('marketplaceOrders.detail.paymentInfo')}
            </h2>
            <div className="space-y-2 text-sm">
               <div className="flex justify-between">
                 <span className="text-slate-500">{t('marketplaceOrders.detail.method')}</span>
                 <span className="font-medium translate-y-[1px]">{orderData.paymentMethod || t('marketplaceOrders.detail.unknown')}</span>
               </div>
               <div className="flex justify-between">
                 <span className="text-slate-500">{t('marketplaceOrders.detail.status')}</span>
                 <span className={`font-medium px-2 py-0.5 rounded text-xs ${orderData.paymentStatus === 'PAID' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}>
                   {t(`finance.status.${orderData.paymentStatus}`, orderData.paymentStatus ?? '—')}
                 </span>
               </div>
            </div>
          </div>
